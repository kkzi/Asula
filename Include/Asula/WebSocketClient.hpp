...
namespace ws
{
    ...
    Url UrlFromString(const std::string& url)
    {
        ...
        if (hasColon)
        {
            ...
            std::string portStr = temp.substr(colonPos + 1, hasSlash ? slashPos - colonPos : -1);
            try
            {
                u.port = NumberCast::fromString<uint16_t>(portStr);
            }
